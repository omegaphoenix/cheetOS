...
int filter_command_line_args(Command *command,
    ...
    for (iter = 0; iter < size_of_array; iter++) {
        switch (tokens[iter]) {
            ...
            /*
             * Empty statement after case, because you can't initialize
             * variable immediately after a label. Command word initialized
             * above
             */
            case WORD: ;
            ...
            case OUT_REDIR: ;
                ...
                Redirection *new_redirect =
                    Redirection_new_pointer(redirect_type,
                                            command_line[iter]);
